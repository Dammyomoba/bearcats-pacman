...
class Player extends Actor {
  constructor(r,c) {
    super(r,c);
    this.heldDir   = null;  // direction currently held (null = no key held)
    this.queuedDir = null;  // direction change queued mid-glide
    this.glideT    = 0;     // glide progress 0..1
    this.glideFrom = null;  // pixel start [px,py] of current glide
  }

  pressDir(d) {
    this.heldDir   = [d[0], d[1]];
    this.queuedDir = [d[0], d[1]];
  }
  ...
  step(speed) {
    // ── Mid-glide: advance smooth animation ───────────────────────────────
    if (this.nextCell) {
      // glideSpeed: fraction of cell per frame — tune for feel
      // CELL=20px, 60fps → glideSpeed=0.25 means 4 frames (~67ms) per cell
      const glideSpeed = 0.28;
      this.glideT = Math.min(1, this.glideT + glideSpeed);
      const tx = this.nextCell[1]*CELL + CELL/2;
      const ty = this.nextCell[0]*CELL + CELL/2;
      this.px = this.glideFrom[0] + (tx - this.glideFrom[0]) * this.glideT;
      this.py = this.glideFrom[1] + (ty - this.glideFrom[1]) * this.glideT;

      if (this.glideT >= 1) {
        // Arrived — snap exactly to cell centre
        this.px = tx; this.py = ty;
        this.r  = this.nextCell[0]; this.c = this.nextCell[1];
        this.nextCell = null; this.glideT = 0; this.glideFrom = null;
        // Immediately try next cell so there is zero pause between steps
        this._tryMove();
      }
      return;
    }
    // ── At rest — try to move if key is held ─────────────────────────────
    this._tryMove();
  }
  ...
  _tryMove() {
    ...
    if (walkable(nr, nc)) {
      this.glideFrom = [this.px, this.py];
      this.glideT    = 0;
      this.nextCell  = [nr, nc];
      this.dir       = [dr, dc];
      // Kick off first frame of glide immediately
      const glideSpeed = 0.28;
      this.glideT = Math.min(1, this.glideT + glideSpeed);
      const tx = nc*CELL + CELL/2, ty = nr*CELL + CELL/2;
      this.px = this.glideFrom[0] + (tx - this.glideFrom[0]) * this.glideT;
