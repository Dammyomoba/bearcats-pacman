...
class Player extends Actor {
  constructor(r,c) { super(r,c); this.queued=[0,0]; }
  setDir(d) { if(d[0]!==0||d[1]!==0) this.queued=[d[0],d[1]]; }
  step(speed) {
    if (this.nextCell) {
      if (!this.advance(speed)) return;
      this.nextCell = null;
    }
    for (const tryd of [this.queued, this.dir]) {
      if (tryd[0]===0&&tryd[1]===0) continue;
      const nr=this.r+tryd[0], nc=this.c+tryd[1];
      if (walkable(nr,nc)) {
        this.nextCell=[nr,nc]; this.dir=[tryd[0],tryd[1]]; this.advance(speed); return;
      }
    }
    this.dir=[0,0];
  }
...
class Game {
  ...
  _logSample(outcome) {
    ...
    } else if (this.player.queued[0]!==0||this.player.queued[1]!==0) {
      chosen=dname(this.player.queued);
    }
    const e={
...
function init({canvas, numGhosts, speed, onEnd}) {
  ...
  function kup(e){ held.delete(e.key); }
  window.addEventListener('keydown', kdown);
  window.addEventListener('keyup',   kup);

  function loop(nowMs){
    let hd=[0,0];
    if(held.has('ArrowUp')   ||held.has('w')||held.has('W')) hd=[-1,0];
    else if(held.has('ArrowDown') ||held.has('s')||held.has('S')) hd=[1,0];
    else if(held.has('ArrowLeft') ||held.has('a')||held.has('A')) hd=[0,-1];
    else if(held.has('ArrowRight')||held.has('d')||held.has('D')) hd=[0,1];
    if(hd[0]||hd[1]) game.player.setDir(hd);
  ...
  return {
    game,
    stop(){ if(raf)cancelAnimationFrame(raf); window.removeEventListener('keydown',kdown); window.removeEventListener('keyup',kup); }
  };
}
