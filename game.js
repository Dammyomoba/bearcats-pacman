...
class Player extends Actor {
  constructor(r,c) {
    super(r,c);
    this.pending = null;   // set by ONE keypress; cleared after consumed
  }
  pressDir(d) {
    // Called once per KEYDOWN event — stores the direction for next step
    if (d[0]!==0||d[1]!==0) this.pending = [d[0],d[1]];
  }
  step(speed) {
    // If still gliding toward next cell, keep the animation going
    // DO NOT start any new move until fully arrived
    if (this.nextCell) {
      this.advance(speed);
      return;
    }
    // Fully arrived at a cell centre — consume pending press if any
    if (!this.pending) return;   // no key was pressed — stay still
    const [dr, dc] = this.pending;
    this.pending = null;         // consume — one press = one step, then stop
    ...
    // Blocked wall — press consumed silently, player stays put
  }
}
...
function init({canvas, numGhosts, speed, onEnd}) {
  ...
  // Key map: arrow keys and WASD → direction vector
  const KEY_DIR = {
  ...
  function kdown(e){
    ...
    // e.repeat is true when the key is HELD DOWN — ignore these
    // This ensures only a fresh physical keypress registers
    if(e.repeat) return;
    if(game.status !== 'IN_PROGRESS') return;
    // One physical keypress = one cell only
    if(KEY_DIR[e.key]) game.player.pressDir(KEY_DIR[e.key]);
  }
  window.addEventListener('keydown', kdown);

  function loop(nowMs){
    // No key polling here — movement driven purely by keydown events above
    game.update();
  ...
  return {
    game,
    stop(){ if(raf)cancelAnimationFrame(raf); window.removeEventListener('keydown',kdown); }
  };
}
