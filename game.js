...
class Player extends Actor {
  constructor(r,c) {
    ...
    this.lastDir  = null;  // last direction pressed (for queued turns)
    this.tickAcc  = 0;     // accumulator: counts px moved this tick
  }
  ...
  step(speed) {
    // No key held — stay still, reset accumulator
    if (!this.heldDir && !this.lastDir) return;

    const moveDir = this.lastDir || this.heldDir;

    // Accumulate speed pixels this frame
    this.tickAcc += speed;

    // Only snap to next cell once we have accumulated a full CELL worth
    if (this.tickAcc < CELL) return;
    this.tickAcc -= CELL;   // keep remainder for smooth carry-over
    ...
    // Consume lastDir after one step — continue only if key still held
