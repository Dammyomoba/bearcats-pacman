...
class Player extends Actor {
  constructor(r,c) {
    super(r,c);
    this.pendingDir = null;  // set by ONE keypress, cleared after one move
    this.moved      = false; // true once we took a step this keypress
  }

  // Called on physical keydown (e.repeat filtered out in init)
  pressDir(d) {
    this.pendingDir = [d[0], d[1]]; // store for next update
    this.moved      = false;        // allow one move per press
  }

  releaseDir(d) {
    // Nothing needed — movement is controlled by pendingDir, not held state
  }

  step(speed) {
    // Only move if there is a pending direction AND we haven't moved yet
    if (!this.pendingDir || this.moved) return;

    const [dr, dc] = this.pendingDir;
    const nr = this.r + dr, nc = this.c + dc;

    if (walkable(nr, nc)) {
      // Snap to next cell instantly — one cell per keypress
      this.r   = nr; this.c   = nc;
      this.px  = nc * CELL + CELL / 2;
      this.py  = nr * CELL + CELL / 2;
      this.dir = [dr, dc];
    }
    // Mark as moved — no more movement until next keypress
    this.moved      = true;
...
function init({canvas, numGhosts, speed, onEnd}) {
  ...
  function kdown(e){
    ...
    // Block OS key-repeat — only fresh physical presses count
    if(e.repeat) return;
    if(game.status !== 'IN_PROGRESS') return;
    // One physical keypress = one cell move only
