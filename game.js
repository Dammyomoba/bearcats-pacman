...
// ── Human player ──────────────────────────────────────────────────────────────
// Uses the exact same movement model as Ghost below: sit at a cell, decide
// the next cell to head into, then glide toward it smoothly via
// Actor.advance() at `speed` px/frame — identical motion feel and identical
// speed value to the ghosts, just driven by held keys instead of A*.
class Player extends Actor {
  constructor(r,c) {
    super(r,c);
    this.heldDir = null;  // direction of key currently held down
    this.lastDir = null;  // queued turn: try this direction first at the next cell boundary
  }

  pressDir(d) {
    this.heldDir = [d[0], d[1]];
    this.lastDir = [d[0], d[1]];
  }

  releaseDir(d) {
    if (this.heldDir &&
        this.heldDir[0]===d[0] && this.heldDir[1]===d[1]) {
      this.heldDir = null;
    }
  }

  _pickNextCell() {
    const tryDir = this.lastDir || this.heldDir;
    if (!tryDir) return;
    const [dr, dc] = tryDir;
    if (walkable(this.r + dr, this.c + dc)) {
      this.dir = [dr, dc];
      this.nextCell = [this.r + dr, this.c + dc];
    }
    // Consume the queued turn either way: retry with whatever's currently
    // held on the next cell-boundary decision (mirrors the old queued-turn
    // buffering — a direction pressed slightly early still takes effect
    // right as the player reaches the intersection).
    this.lastDir = this.heldDir;
  }

  step(speed) {
    if (!this.nextCell) this._pickNextCell();
    if (this.nextCell) this.advance(speed);
  }
}
