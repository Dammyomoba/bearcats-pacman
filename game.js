...
class Player extends Actor {
  constructor(r,c) {
    super(r,c);
    this.heldDir   = null; // direction key currently held down
    this.queuedDir = null; // direction pressed mid-step, used on next step
    this.stepReady = true; // true when player is at a cell and ready to move
    this.stepTimer = 0;    // frame counter — controls step rate to match ghosts
  }
  ...
  step(speed) {
    // Use same timing as ghosts: ghosts move speed px/frame over CELL px
    // so one cell takes CELL/speed frames. We count frames to match.
    const framesPerCell = Math.round(CELL / speed);

    this.stepTimer++;
    if (this.stepTimer < framesPerCell) return; // not time to move yet
    this.stepTimer = 0;                          // reset timer, take a step

    const moveDir = this.queuedDir || this.heldDir;
    this.queuedDir = null;
    if (!moveDir) return; // no key held — stay still

    const [dr, dc] = moveDir;
    const nr = this.r + dr, nc = this.c + dc;
    if (walkable(nr, nc)) {
      // Snap instantly to the next cell — no glide
      this.r  = nr; this.c  = nc;
      this.px = nc * CELL + CELL / 2;
      this.py = nr * CELL + CELL / 2;
      this.dir = [dr, dc];
    }
    // Blocked: stay in place, try again next step
  }
}
