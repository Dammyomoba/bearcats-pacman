...
class Player extends Actor {
  ...
  step(speed) {
    // If still gliding between cells, keep animating — do NOT start a new move
    if (this.nextCell) {
      if (!this.advance(speed)) return;   // still mid-glide
      this.nextCell = null;               // arrived at cell centre
    }
    // Only move if a key was pressed AND we are now at a cell centre
    if (!this.pending) return;            // no key pressed — stand still
    const [dr,dc] = this.pending;
    this.pending = null;                  // consume the press — one press = one cell
    ...
    // If blocked, the press is simply consumed and nothing happens
  }
}
...
function init({canvas, numGhosts, speed, onEnd}) {
  ...
  function kdown(e){
    ...
    // One keypress = one cell move queued — no auto-repeat from holding
